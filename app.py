...
@st.cache_data(show_spinner="A calcular a topologia matemática e extrair métricas...")
def obter_dataframe_metricas(dados_recorte):
    """Gera o DataFrame de forma 100% independente, otimizado para matemática."""
    G = nx.Graph()
...
@st.cache_resource(show_spinner="A construir a rede visual interativa...")
def gerar_html_pyvis(dados_recorte):
    """Gera o arquivo HTML do Pyvis com injeção de JavaScript, independente da tabela."""
    G = nx.Graph()
    for tese in dados_recorte:
        doc_id = tese['titulo']
    ...
    # Métricas para os balões de Hover
    degree_cent = nx.degree_centrality(G)
    betweenness_cent = nx.betweenness_centrality(G)

    for node, attrs in G.nodes(data=True):
        tipo = attrs.get('tipo', 'Desconhecido')
        janela_sna = f"<hr><b>📊 Métricas SNA:</b><br>Grau Absoluto: {G.degree(node)}<br>Degree Centrality: {degree_cent[node]:.4f}<br>Betweenness: {betweenness_cent[node]:.4f}"
        
        if tipo == 'Documento':
            attrs.update({'color': '#E74C3C', 'shape': 'square', 'size': 30, 'title': f"<b>Tese:</b><br>{node}<br><b>Ano:</b> {attrs.get('ano')}{janela_sna}"})
        elif tipo == 'Autor':
            attrs.update({'color': '#3498DB', 'shape': 'dot', 'size': 20, 'title': f"<b>Autor:</b><br>{node}{janela_sna}"})
        elif tipo == 'Orientador':
            attrs.update({'color': '#F39C12', 'shape': 'star', 'size': 25, 'title': f"<b>Orientador:</b><br>{node}{janela_sna}"})
        elif tipo == 'Conceito':
            attrs.update({'color': '#2ECC71', 'shape': 'triangle', 'size': 15, 'title': f"<b>Conceito:</b><br>{node}{janela_sna}"})
    ...
    # Injeção JavaScript para ocultar nós ao clicar
    script_ocultar = """
...
with st.form("form_grafo"):
    ...
    with col_g1:
        n_registros_grafo = st.slider("Quantidade de Documentos para a Rede Visual (Cuidado com redes > 200 nós):", 5, total_documentos, 40, 5)
    with col_g2:
        st.markdown("<br>", unsafe_allow_html=True)
        btn_render_grafo = st.form_submit_button("Renderizar Grafo", use_container_width=True)

if btn_render_grafo:
    path, nos, arestas = gerar_html_pyvis(dados_completos[:n_registros_grafo])
...
if st.session_state['grafo_pronto']:
    ...
    c4.info("Dica: Clique em um nó para isolá-lo.")
...
with st.form("form_tabela"):
    ...
    with col_t1:
        n_registros_tabela = st.slider("Documentos analisados matematicamente (Pode usar a base completa!):", 5, total_documentos, total_documentos, 5)
    with col_t2:
...
if st.session_state['tabela_pronta']:
    ...
    # Formatação visual para não ficar feio na tela
