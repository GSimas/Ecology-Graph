...
with st.sidebar:
    ...
    st.markdown(
        "O **EcoGrad** é uma plataforma analítica que mapeia e visualiza as redes de produção acadêmica da Pós-Graduação (Teses e Dissertações) e Graduação (TCCs)."
    )
    st.markdown("### ⚙️ Como Funciona")
    st.markdown(
        "Os documentos são processados usando algoritmos de redes complexas (Ciência de Redes) aliados a Inteligência Artificial. Isso desvela como pesquisadores, teorias e ferramentas se interconectam na academia."
    )
    st.markdown("### 🧭 Como Utilizar")
    st.markdown(
        "1. **Selecione os cursos** e origens na aba principal.\n2. Navegue pelos **Dashboards** e visualize perfis no Motor de Busca.\n3. Explore os dados de um Autor, Orientador ou Conceito.\n4. Acesse **Análises Avançadas** para métricas detalhadas."
    )
    st.markdown("---")
    st.markdown(
        "Desenvolvido por **Gustavo Simas**<br>[🔗 GitHub: GSimas](https://github.com/GSimas)",
...
if "dados_completos" not in st.session_state or not st.session_state["dados_completos"]:
    ...
    st.info(
        "Por favor, vá à página inicial e carregue a base de um Programa de Pós-Graduação antes de iniciar o chat."
    )
    st.stop()
...
if not api_key:
    st.error(
        "⚠️ Erro: GEMINI_API_KEY não encontrada em variáveis de ambiente ou secrets."
    )
    st.stop()
...
def preparar_contexto_academico(dados, nome_prog):
    ...
    time_container.info(
        f"⏱️ **Tempo estimado de processamento:** ~{tempo_estimado} segundos."
    )
    bar = st.progress(0)
    status_text = st.empty()

    status_text.markdown(
        "⏳ **Passo 1/4:** Construindo a Topologia da Rede Acadêmica..."
    )
    ...
    bar.progress(40)
    status_text.markdown(
        "⏳ **Passo 2/4:** Calculando Centralidade de Grau (Degree Centrality)..."
    )
    deg_cent = nx.degree_centrality(G)

    bar.progress(60)
    status_text.markdown(
        "⏳ **Passo 3/4:** Mapeando Pontes Interdisciplinares (Betweenness Centrality)..."
    )
    ...
    bar.progress(85)
    status_text.markdown(
        "⏳ **Passo 4/4:** Redigindo Dossiê Comportamental para a IA..."
    )
    ...
    ctx += (
        "Líderes em Volume de Orientação (Degree Centrality): "
        + ", ".join([f"{n}" for n, v in top_oris_vol])
        + "\n"
    )
    ctx += (
        "Pontes Interdisciplinares (Betweenness Centrality - Conectam diferentes áreas): "
        + ", ".join([f"{n}" for n, v in top_oris_ponte])
        + "\n"
    )
    ctx += (
        "Principais Conceitos Pesquisados: "
        + ", ".join([f"{n}" for n, v in top_conceitos])
        + "\n\n"
    )

    ctx += "--- PERFIL DE ORIENTAÇÃO (MAPA DE ESPECIALISTAS) ---\n"
    for ori, info in sorted(
        perfil_ori.items(), key=lambda x: x[1]["total"], reverse=True
    ):
    ...
    for d in dados[:1500]:
        ...
        # Puxa a URL do documento, se existir
        url = d.get("url", "")
        url_info = f" | LINK: {url}" if url else ""

        ctx += f"TÍTULO: {d.get('titulo')} | AUTOR: {', '.join(d.get('autores', []))} | ORIENTADOR: {d.get('orientador')} | TEMA: {d.get('macrotema')} | CONCEITOS: {pks}{url_info}\n"
...
st.caption(
    f"Alimentado por Gemini Flash | Especialista em: {nome_programa} | Sempre verifique as respostas, o chatbot pode ser impreciso"
)
st.markdown(
    "Bem-vindo! Descreva sua ideia de projeto, pergunte sobre o perfil dos professores, ou peça indicações de teses alinhadas com seu interesse de pesquisa."
)
...
if prompt := st.chat_input(
    ...
    for m in st.session_state.messages[
        :-1
    ]:  # Exclui a última (que acabamos de adicionar)
